...
const MissionModify = ({ missionId, onUpdate }) => {
    ...
    const handleUpdate = async () => {
        try {
            ...
            // await axios.patch(`http://localhost:8080/api/missions/${missionId}`, formData);
            await ApiClient.patch(`/missions/${missionId}`, formData)
                .then((response) => {
                    console.log(response);
                    console.log("수정성공");
                })
            ...
            >
                ...
                <CModalBody>
                    ...
                    <CFormInput type="file" id="formFile" label="이미지" accept='image/*'
                        value={image} name='image' multiple="true" onChange={handleImageChange}
                    />
                    <br></br>
